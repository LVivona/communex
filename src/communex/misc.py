...
def get_global_params(c_client: CommuneClient) -> NetworkParams:
    ...
    adjustment_interval = burn_config["adjustment_interval"]
    expected_registrations = burn_config["expected_registrations"]
...
def local_keys_to_stakedbalance(
    c_client: CommuneClient, netuid: int = 0
) -> dict[str, int]:
...
def local_keys_allbalance(
    c_client: CommuneClient, netuid: int | None = None
